...
def step_01_eda():
    ...
    print("\n" + "="*60)
    print("EXECUTANDO: 01 - Analise Exploratoria e Montagem de Base")
    print("="*60)
    ...
    try:
        ...
        # Detecta eventos
        events = get_events()
        print(f"[OK] Eventos detectados: {events}")
        
        return True
        
    except Exception as e:
        print(f"[ERRO] Erro na etapa 01: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def step_02_cleaning():
    ...
    try:
        ...
        print(f"[ERRO] Erro na etapa 02: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def step_03_analysis():
    ...
    print("\n" + "="*60)
    print("EXECUTANDO: 03 - Analise de Negocio e Dashboard")
    print("="*60)
    ...
    try:
        ...
        print(f"[OK] Dados carregados: {len(sales)} vendas, {len(allocated)} registros alocados")
        ...
        print(f"[OK] KPIs calculados: {len(kpis)} meses")
        print(f"[OK] Inadimplencia: {len(defaults_t)} periodos")
        ...
        print("\nANALISE 1 - Mudanca Estrategica (Jan/2021):")
        ...
        print("\nANALISE 2 - Expansao Recorrente (Jul/2021):")
        ...
        if events['whatsapp_start']:
            ...
            print("\n[AVISO] WhatsApp: data nao detectada no PDF")
        ...
        print("\nANALISE 4 - Produtos Premium (Ago/2024):")
        pre = premium.get("pre")
...
{plots}
        ...
        print(f"\n[OK] Dashboard gerado: {dashboard_path}")
        
        return True
        
    except Exception as e:
        print(f"[ERRO] Erro na etapa 03: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def step_04_report():
    ...
    print("\n" + "="*60)
    print("EXECUTANDO: 04 - Geracao do Relatorio Final")
    print("="*60)
    ...
    try:
        ...
        print(f"\n[OK] Relatorio gerado: {report_path}")
        ...
        print(f"[ERRO] Erro na etapa 04: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    print("INICIANDO ANALISE COMPLETA - TOURISM COMPANY DELINQUENCY")
    print("="*80)
    ...
    for name, func in steps:
        ...
        if not success:
            print(f"[ERRO] Falha na etapa: {name}")
            break
    ...
    if all(success for _, success in results):
        ...
        print(f"Relatorio: file:///{REPORTS_DIR / 'business_report.md'}")
        return True
    else:
        print("\n[ERRO] ANALISE FALHOU EM ALGUMAS ETAPAS")
        return False
